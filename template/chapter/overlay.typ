...
*power-law and scale-free networks*. These topologies deliberately introduce
...
- *Fairness and incentive alignment.* Most protocols assume cooperative participants, yet real-world systems must account for rational, self-interested nodes. Designing overlays that align individual incentives with global efficiency, without resorting to heavy consensus or tokenomics, remains largely open outside the blockchain domain.
- *Physical-topology awareness.* Abstract overlays often ignore the underlying network geometry. Adapting peer sampling to respect physical latency, bandwidth constraints, or administrative boundaries (e.g., ISP routing policies) without sacrificing decentralization is a persistent unsolved problem.
- *Bootstrapping and network entry.* The initial join process---how a new node securely and efficiently discovers its first peers without relying on fragile centralized bootstrappers or out-of-band coordination---remains a critical bottleneck in long-lived P2P architectures.
- *Resource efficiency and edge deployment.* The communication, computational, and energy overhead of overlay maintenance must be drastically reduced to support deployment on edge devices, IoT gateways, or low-capacity nodes operating under intermittent connectivity and strict power budgets.
- *Decentralized hub election and formation.* While many protocols actively cap or limit hub influence, few address how a network can autonomously select and sustain a predefined number of hubs without central coordination or global state, while guaranteeing fair opportunity for hub emergence and resilience to targeted removals.

Among these open research directions, this thesis deliberately narrows its scope to the challenge of *decentralized hub election and formation*. Rather than attempting to solve all overlay management problems simultaneously, our work specifically targets the gap in controlled, application-parameterized hub emergence. The contribution presented in @chap:elevator addresses this precise challenge by proposing a fully distributed mechanism that steers the overlay toward a target number of hubs, guarantees bounded recovery times after hub failures, and maintains stability under high churn. The broader challenges outlined above remain open and constitute natural extensions for future work building upon the foundations established in this thesis.