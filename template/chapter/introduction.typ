...
Yet the open and decentralized architecture of the Internet did not prevent the emergence of centralization at the layer of services built on top of it. The protocols that gave rise to the World Wide Web were open and accessible to all, but the services that came to dominate the Web were not. Over the course of two decades, a small number of technology companies --- today grouped under the informal label of Big Tech --- came to occupy a position of structural dominance over the digital lives of billions of people. Their success rested on a straightforward logic: a centralized service, operated from a single infrastructure, can serve the entire planet while continuously improving through the aggregation of its users' data. The simplicity they offered was genuine, and the economies of scale they achieved were real. But these advantages came at a price that was not always made explicit: the progressive transfer of personal data, behavioral traces, and, ultimately, a degree of individual autonomy to private entities whose decisions remain largely opaque.

In response to this concentration, communities of developers and researchers began, from the early days of the Web, to imagine and build alternatives. Peer-to-peer protocols --- systems in which participants interact directly with one another rather than through a central intermediary --- offered a different vision of what networked services could look like: distributed, egalitarian, and resilient by design. Some of these efforts achieved remarkable reach; others remained confined to niche communities. But together, they demonstrated that decentralized alternatives were technically feasible, and they raised questions of sufficient depth and generality to attract the attention of the academic research community. How does a system coordinate without a center? How does it remain reliable when its participants are unreliable? How does it scale when no single entity oversees its growth? These questions sit at the intersection of computer science, mathematics, and the theory of complex systems, and they remain active areas of inquiry today.

Beyond technical architecture, the question of whether digital infrastructure should be centralized or decentralized is, at its core, a question about power: who controls the systems that mediate communication, commerce, and knowledge; who has access to the data these systems generate; and who bears the risks when they fail or are misused. There is no neutral answer. Centralized services offer convenience and efficiency, but they concentrate power and create dependencies that individuals and institutions may not fully perceive until they are already deeply embedded. Decentralized alternatives demand more from their participants --- a degree of technical literacy, a willingness to share responsibility, and a collective commitment to maintaining the commons --- but they offer in return a form of autonomy and resilience that centralized architectures cannot provide by design. Whether this trade-off is worth making is not a question this thesis attempts to answer. It is an open societal debate, and one that will not be resolved by technical means alone. What this thesis does attempt is to advance the technical foundations that would make decentralized alternatives more viable, so that this choice can be made on informed and practical grounds.
...
Pursuing this question requires bridging two research communities that have, until recently, evolved largely independently of one another. The networking and distributed systems community has developed a rich body of work on peer-to-peer protocols, graph theory, and self-stabilizing systems, and is accustomed to reasoning about well-defined, often deterministic environments: nodes, links, message complexity, consensus protocols, and fault models with precise formal guarantees. The machine learning community operates in a fundamentally different register --- one of statistics, optimization, and empirical validation, where the objects of study are loss functions, gradient estimates, and generalization bounds, and where the notion of a network, if it appears at all, refers to the architecture of a model rather than the topology of a communication system. These two communities do not share the same vocabulary, the same proof techniques, or the same experimental culture. The consequence is that the space where they overlap --- decentralized machine learning over peer-to-peer networks --- has remained comparatively underexplored. Addressing it seriously requires fluency in both domains simultaneously: any theoretical analysis must account for the stochastic dynamics of learning and the adversarial dynamics of distributed coordination at the same time, and any experimental evaluation must be credible from both perspectives.

The motivations for pursuing this research direction extend well beyond technical curiosity. The ethical dimensions are concrete and pressing. At stake are three interrelated concerns: privacy, since centralized training requires raw data to leave the hands of those who generated it and be aggregated under the control of a third party; sovereignty, since the growing dependence of public institutions on models developed and controlled by a small number of private actors creates dependencies that individuals, nations, and democratic systems may find increasingly difficult to accept; and accountability, since a model trained on undisclosed data, by an unreproducible process, and deployed through an opaque interface offers little purchase to the conventional instruments of democratic oversight --- transparency, contestability, and liability. These concerns crystallise around a common pattern: there exist many situations in which no single participant holds enough data to train a useful model on their own, yet every participant has strong reasons --- legal, competitive, or ethical --- not to share that data with a third party. Decentralized learning addresses precisely this tension.

Consider the medical domain. A hospital treating patients with a rare disease may have access to only a handful of relevant cases --- far too few to train a reliable diagnostic model. Neighboring hospitals face the same constraint. Yet patient data is among the most sensitive information that exists, and sharing it with an external party raises serious concerns about confidentiality, consent, and regulatory compliance. A decentralized training framework would allow these institutions to collaboratively build a model of a quality that none of them could achieve alone, without any patient record ever leaving the institution that collected it.

A second case arises among competing actors who share a common interest. Consider the automotive industry, where multiple manufacturers are independently developing autonomous driving systems. Each company guards its data jealously, as it represents a significant competitive asset. Yet all of them share an interest in reducing the rate of accidents --- a goal that improves with the volume and diversity of training data. Decentralized learning makes it possible to aggregate the benefits of a shared training process without requiring any participant to expose proprietary data to its competitors. The result is a model that is safer for everyone, produced by a process that compromises no one.
...
What these cases share is something deeper than a technical property. Decentralized learning is, at its core, an exercise in building a common good. Each participant contributes what they have, retains what they wish to protect, and benefits from the collective result. No single actor controls the outcome, and no single actor can capture it. This vision of collaborative intelligence --- distributed, egalitarian, and resistant to capture --- is what gives the research direction developed in this thesis its broader significance.

Decentralized learning is not the only possible response to these concerns, and it would be misleading to present it as a perfect solution. Several alternative approaches exist, each with its own merits and limitations. One option is to encrypt data before sharing it, using techniques such as homomorphic encryption, which allows computations to be performed directly on encrypted data without ever decrypting it. This approach offers strong theoretical guarantees, but it comes at a computational cost that remains, in practice, prohibitive for training large models. Another option is to add carefully calibrated statistical noise to the data or to the model updates before they are shared --- a technique known as differential privacy --- which makes it mathematically impossible to recover individual records from the shared information. This is a valuable complement to decentralized learning, but it does not by itself address the question of who controls the training process. A third option is to rely on trusted execution environments: specialized hardware enclaves that guarantee, at the chip level, that even the operator of the infrastructure cannot access the data being processed. This provides a technical guarantee without requiring decentralization, but it transfers the question of trust from the service provider to the hardware manufacturer. A fourth option is to place a centralized actor under strict legal obligations, relying on regulation rather than architecture to enforce privacy and accountability. This approach has real value, and the European Union's efforts in this direction --- through the GDPR, the AI Act, and related instruments --- represent a serious attempt to make it work. But a well-intentioned and legally compliant centralized actor remains a single point of failure: it can be compromised by a cyberattack, subject to unconscious biases in how it curates training data, or placed under political or economic pressure that no regulatory framework can fully anticipate. Finally, one might ask whether the problem can be sidestepped altogether --- either by generating synthetic data locally to augment small datasets, or by developing model architectures that require less data to achieve useful performance. These are active and promising research directions, but they do not generalize: for many applications, and in particular for rare or highly specific phenomena, neither synthetic generation nor data-efficient architectures can substitute for the real, distributed data that participants hold.

The appeal of decentralized learning, in this context, lies in the fact that it addresses the problem at the architectural level. Privacy and the absence of central control are not properties that depend on the goodwill of an operator, the robustness of a legal framework, or the security of a hardware supply chain --- they are structural consequences of how the system is designed. This does not mean that decentralized systems are without weaknesses. They introduce their own trade-offs: coordinating training across many independent participants without a central authority is harder, slower, and more vulnerable to certain classes of malicious behavior#footnote[In this thesis, malicious behavior is studied at the overlay level, where an adversary could manipulate the hub election (addressed by the Lift protocol, @chap:elevator). Defending the machine-learning layer itself against Byzantine or poisoning attacks (e.g., manipulated model updates) lies outside the scope of this work; we return to this point in the limitations and outlook (@chap:conclusions_outlook).] than centralized training.

This raises the central question of this thesis: is it possible to build a decentralized learning system that matches the efficiency of its centralized counterparts, while being fully decentralized and resilient to failures and malicious participants? This question is not merely rhetorical --- it is technically non-trivial, and the answer is far from obvious. This thesis attempts to address it through two complementary lenses: a structured review of the existing literature at the intersection of peer-to-peer systems and machine learning, which maps the landscape of prior work and identifies the open problems that motivate the contributions that follow; and the design and evaluation of original protocols for decentralized federated learning, which make deliberate design choices to advance what is achievable in terms of efficiency, decentralization, and resilience against malicious behavior.

== Contributions

The work presented in this thesis is organized around a unifying architectural
view. Decentralized learning of the kind investigated here is not a single
...
}), alt: "Layered architecture of decentralized learning showing four stacked layers: Network, Overlay, Aggregation, and Application", caption: [The four-layer architecture of decentralized learning used throughout this thesis.]
...
learning stacks --- has not been proposed before in the context of decentralized
...
at the intersection of peer-to-peer networking and decentralized machine
learning.

The first contribution is *Elevator*, a decentralized peer sampling protocol that organizes nodes into a hub-and-spoke overlay through a lightweight, self-organizing random election mechanism. Elevator requires no pre-assigned coordinator, makes no assumption about the application running on top of it, and is designed to tolerate node crashes and churn. By elevating a dynamic subset of nodes to the role of hubs, it introduces a structured aggregation topology into an otherwise flat peer-to-peer network, without sacrificing the decentralized nature of the system. Elevator and its analysis were published in @legheraba2024emergent.

The second contribution is *Lift*, an extension of Elevator that addresses Byzantine fault tolerance. Where Elevator assumes that participants follow the protocol honestly, Lift introduces an analysis of the protocol's behavior under Byzantine attacks and equips it with a defense mechanism that limits the influence of malicious participants on the hub election process. Lift strengthens the security guarantees of the entire protocol stack built on top of Elevator. Lift has been published in @legheraba2025lift.

The third contribution is *HEAL*, a decentralized federated learning protocol built directly on top of the Elevator overlay. HEAL exploits the two-level structure of the overlay — hubs aggregating local model updates, and inter-hub communication reconciling aggregates across the network — to recover the convergence efficiency of classical federated learning while preserving full decentralization. HEAL is evaluated under fault-free conditions as well as under crash failures and churn, across multiple model architectures and learning tasks. HEAL has been published in @legheraba2025heal.

The fourth contribution is *FLAIR* , an adaptation of HEAL to the constraints of physical wireless networks. In a WiFi environment, the overlay topology cannot be chosen freely: it is shaped by radio range, interference, and the physical proximity of devices. FLAIR draws inspiration from LEACH-style clustering to align the logical aggregation structure with the physical network layer, making decentralized federated learning deployable in realistic wireless settings. FLAIR has been published in @boutebicha2026netys.

== Organization of the manuscript

The manuscript is organized into six chapters, reflecting the progressive construction of the framework from its theoretical foundations to its applied extensions.

@chap:model introduces the theoretical model that underpins the entire thesis. It formalizes the system assumptions, the network model, and the abstractions used throughout the subsequent chapters.
...
@chap:learning surveys the state of the art on decentralized learning. It introduces the necessary background on federated learning, and gossip learning, and reviews the main approaches to decentralized aggregation in the learning literature.
