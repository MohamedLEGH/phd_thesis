...
// A peer-to-peer (P2P) architecture is a network model in which there is no central entity. All participants in the network generally have equivalent roles and capabilities. This contrasts with the client–server model, where multiple clients rely on a centralised server that coordinates data exchanges and resource management.

// The main advantage of the client–server architecture lies in its simplicity. A central server handles client requests and coordinates all operations, while users only need to connect through a well-defined interface or protocol. Security management is also more centralized, since protecting the server typically protects the entire system. However, this apparent simplicity comes at a significant cost: the server constitutes a single point of failure. If it crashes, the entire service becomes unavailable, and if it is compromised, all clients are exposed. Moreover, the computing load and data storage are concentrated on the server side, while the bandwidth and computational resources of clients remain largely underutilized.
...
// Although today’s digital services (e.g. GAFAM) are mostly based on centralized architectures, the Internet itself was originally conceived as a decentralised system, as it can be seen in the map of the ARPANET network, on @arpanet. 
...
// The first popular peer-to-peer (P2P) applications emerged in the late 1990s, motivated by the growing need to share large multimedia files over the Internet. At that time, computing and networking resources (CPU, memory, bandwidth, and storage) were limited, making it difficult for a single server to handle massive numbers of simultaneous downloads.
