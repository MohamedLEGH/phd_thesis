...
This thesis was motivated by a fundamental question: can federated learning be conducted in a truly decentralized manner? The initial intuition pointed toward blockchain as a natural substrate for decentralized coordination. However, a systematic survey of the state of the art revealed an existing paradigm — gossip learning — that already eliminates the central server. Yet gossip learning generally underperforms federated learning, both in convergence speed and, in most settings, in final model accuracy, precisely because it lacks any form of structured aggregation. This underperformance is not, however, systematic: the literature reports instances in which gossip learning can reach the convergence quality of federated learning once the algorithms are adequately configured (@chap:learning). In practice, achieving competitive behavior with gossip learning thus requires a comparatively careful configuration of parameters, which makes it a less reliable substrate for recovering the efficiency of a centralized reference. By contrast, because our approach introduces structured global aggregation on top of a decentralized overlay, it relies less on such delicate configuration and we are correspondingly more confident of approaching the convergence speed of federated learning. This observation shifted the research question to a lower level of the stack: rather than modifying the learning algorithm itself, we asked whether the underlying communication topology could be engineered to recover the benefits of structured aggregation, while preserving full decentralization. This led us to survey the peer sampling and overlay management literature, in search of protocols capable of shaping the network toward a topology that would support efficient global aggregation.
...
performance — and do not consider decentralized learning as a target use case.
More broadly, very few peer sampling protocols have been designed with
decentralized machine learning as an explicit operational context, as documented
in @chap:overlay.
...
server coordinates model aggregation; the question of full decentralization
...
decentralized, yet their lack of global aggregation generally leads to
...
decentralization, scalability, and aggregation efficiency.

This gap shaped the central research question of the thesis: _how can the efficiency of gossip learning be improved so as to approach federated learning, without sacrificing decentralization?_ 
...
The result is *Elevator*, a decentralized peer-to-peer overlay protocol that organizes nodes through a lightweight, self-organizing random election mechanism. Crucially, Elevator is application-agnostic: it provides a general-purpose structured overlay that any distributed algorithm can exploit.
The evaluation of Elevator rests on three complementary pillars. First, a theoretical analysis characterizes the protocol's behavior in terms of overlay properties — diameter, average shortest path length, and clustering coefficient — and derives analytical bounds on convergence and hub election dynamics. Second, these theoretical results are validated through large-scale simulations conducted on PeerSim, a peer-to-peer network simulator widely used and recognized by the research community. Third, a real implementation over TCP/IP confirms that the protocol behaves as predicted at the network level, bridging the gap between simulation and deployment. The consistency across all three levels of evaluation strengthens confidence in the correctness and robustness of the protocol.

This multi-level methodology did not come for free. Peer-to-peer networks with emergent hub structures of the kind introduced by Elevator are a novelty, and the research community has not converged on standard evaluation practices for such systems. A substantial methodological effort was therefore devoted to establishing appropriate scientific rigor: selecting and justifying the evaluation metrics, designing visualizations that faithfully represent the structural properties of the overlay, and adapting PeerSim — a simulator that is over twenty years old and was not designed with hub-based overlays in mind — to the requirements of this work. In particular, scaling simulations to networks of thousands or even hundreds of thousands of nodes, which is the operational target of a general-purpose peer-to-peer protocol, demanded significant engineering effort to make PeerSim tractable at that scale. The evaluation framework developed in the course of this work — comprising the
selection of appropriate overlay metrics, the design of visualizations adapted to
...
in Elevator reveals that the protocol withstands a range of adversarial behaviors
without modification — including passive byzantines that return empty neighbor
...
returning fellow byzantines in their neighbor lists — as few as two percent of
...
dedicated defense mechanism at the overlay level, extending the collusion tolerance
threshold to ten percent of byzantine nodes while preserving the decentralized
...
framework for decentralized, efficient, and resilient machine learning over
peer-to-peer networks. The progression from the state of the art, through Elevator, to HEAL, Lift, and FLAIR constitutes a principled answer to the motivating question: structured aggregation can be achieved without centralization, provided that the overlay layer is designed with that goal in mind.
...
approximate the idealized setting of federated learning: data distributions that are
...
gap with centralized federated learning while operating without any coordinator.
...
without centralization is achievable and practically effective, and they identify
...
which has largely taken centralization for granted — and the gossip learning
...
effort toward truly decentralized, efficient, and trustworthy machine learning over
...
The theoretical analysis of Elevator's convergence relies on idealized assumptions:
...
yield a more realistic characterization of the protocol's behavior and remains an
...
sophisticated adversarial behaviors — such as coordinated collusion or adaptive
...
protocol's behavior under realistic wide-area network conditions.
...
real-world decentralized learning deployments and would allow a more rigorous
...
insufficient to fully account for the variance introduced by random initialization
...
synchronization primitive — remains an open challenge, and would remove the
implicit dependency on an external coordination mechanism, whether centralized
or distributed.
...
the behavior of the protocol under non-IID conditions remains an open question,
...
characterized. Beyond these shared limitations, the network model underlying the
...
experimental setup. While ns-3 provides higher-fidelity modeling of wireless
channel conditions than the simulators used for HEAL, the evaluation still
abstracts away several aspects of real wireless network behavior that could
...
decentralized nature of the process. Preliminary results are encouraging and
...
Elevator to the initial network topology. The current protocol is initialized from a
...
systematic study of this convergence behavior is underway. In parallel, ongoing
...
compromising its structural guarantees. These optimizations are in progress and
...
personalization or locally-weighted aggregation — to mitigate this effect is a
...
accessible experimentation platform for future work in decentralized learning.
...
inference. Equipping HEAL with defenses against these threat models — whether
...
made in the simulation studies — in particular the behavior of the protocol at
...
In the longer term, several directions define a broader research program. First,
...
significantly broaden its applicability, particularly for settings where labeled data
...
in a decentralized manner. One approach is to design such a mechanism as a
...
with the trust and incentive guarantees of a decentralized ledger.
...
industrial deployment of fully decentralized machine learning.
